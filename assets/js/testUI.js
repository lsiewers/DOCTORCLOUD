...
function setUI() {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    if (document.querySelector('.results')) {
      ...
      const currentLang = window.location.pathname.split( '/' )[2] === 'test' ? null : 'eng';
      const urlString =  '//www.doctorcloud.nl/';

      // add button to result page
      document.querySelectorAll('.results .scale').forEach(item => {
        item.classList.forEach(itemClass => {
          if(itemClass.startsWith('scale-slug')) {
            const el = document.createElement('a');
            el.setAttribute('href', `${urlString + currentLang + '/' + itemClass.replace('scale-slug-', '') + (currentLang == "en"?"-2":"") + "/"}`)
            item.prepend(el);
          };
        });
      });
